...
def predict_2024_season_for_ind_lstm(model, prepared_df, scaler, le, sequence_length=6):
    """
...
def predict_2024_season_for_sys_lstm(model, prepared_df, scaler, le, sequence_length=6):
    """
    Predict 2024 season using prepared data
    """
    predictions = []
    ...
    for week in weeks_2024:
        ...
        current_players = prepared_df[
            (prepared_df['season'] == 2024) & 
            (prepared_df['week'] == week)
        ]['original_role_id'].unique()
        
        for player in current_players:
            # Get historical sequence
            player_history = prepared_df[
                (prepared_df['original_role_id'] == player) & 
                ((prepared_df['season'] < 2024) | 
                 ((prepared_df['season'] == 2024) & (prepared_df['week'] < week)))
            ].tail(sequence_length-1)
            ...
            # Combine sequence
            sequence = pd.concat([player_history, current_week])
            sequence_features = sequence.drop(['role', 'recent_team', 'opponent_team', 'fantasy_points', 'original_role_id', 'week', 'season'], axis=1)
            ...
            predictions.append({
                'role_id': player,
                'role': current_week['role'].values[0],
                'recent_team': current_week['recent_team'].values[0],
                'opponent_team': current_week['opponent_team'].values[0],
                'season': current_week['season'].values[0],
                'week': week,
