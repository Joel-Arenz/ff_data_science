...
def prepare_features(df):
    df = df.drop(columns=['completions', 'player_display_name', 'attempts', 'passing_yards', 'sacks', 'sack_yards', 'depth_team',
                          ...
                          'passing_2pt_conversions', 'passing_epa', 'rushing_epa', 'receiving_epa', 'position_encoded', 
                          'recent_team', 'opponent_team', 'position', 'home_score', 'away_score', 'recent_team_points_scored', 
                          'opponent_team_points_allowed', 'turnover', 'points_total', 'yards_total', 'epa_total'])  
    return df
...
def prepare_output(df):
    df = df[['player_id', 'season', 'week', 'player_display_name', 'position', 'recent_team', 'opponent_team', 'fantasy_points']].copy()
    return df